...
import { View, Text, TouchableWithoutFeedback, Image, WebView } from 'react-native';
...
export default function renderNode(node, index, siblings, parent, defaultRenderer) {
  ...
  if(node.name == 'h1' || node.name == 'h2' || node.name == 'h3' || node.name == 'h4' || node.name == 'h5' || node.name == 'h6') {
    if(node.children) {
      return <Text style={textStyle().heading}>{node.children["0"].data}</Text>
    }
  }
